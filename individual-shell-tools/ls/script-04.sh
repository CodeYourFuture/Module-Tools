...
# ls - lists files in the child-directory directory.
# 1 - displays every file on a new line.
# -t - sorts files by modofocation time (the most recently modified file is first).
...
# r - reverses the order of files (oldest first).